...
class curve {
public:
    // Coefficients of the conic curve
    num a, b, c, h, g, f;
...
public:
    ...
    ellipse(const point &c, num semi_a, num semi_b)
        : a(semi_a), b(semi_b), center(c),
          Curve(1 / (semi_a * semi_a), 1 / (semi_b * semi_b), -1, 0, -2 * c.x / (semi_a * semi_a), -2 * c.y / (semi_b * semi_b)),
...
public:
    ...
    point focus;
    point vertex;
    // Eccentricity is always 1 for a parabola
    num eccentricity;
...
public:
    ...
    hyperbola(const point &c, num semi_a, num semi_b)
        : a(semi_a), b(semi_b), center(c),
          eq(1 / (semi_a * semi_a), -1 / (semi_b * semi_b), -1, 0, -2 * c.x / (semi_a * semi_a), -2 * c.y / (semi_b * semi_b)),
          eccentricity(sqrt(1 + ((semi_b * semi_b) / (semi_a * semi_a)))) {}
...
bool ispoint(curve eq, point p) {
    return ((eq.a * p.x * p.x) + (2 * eq.h * p.x * p.y) + (eq.b * p.y * p.y) + (2 * eq.g * p.x) + (2 * eq.f * p.y) + eq.c) == 0;
}
